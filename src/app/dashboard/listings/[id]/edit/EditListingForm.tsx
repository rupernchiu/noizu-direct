...
import { parseShippingMap, type ShippingRateMap } from '@/lib/shipping'
...
export function EditListingForm({ product }: { product: Product }) {
  ...
  // Per-product shipping (Shipping V2). Rates live only on Product.
  const initialShipMap = parseShippingMap(product.shippingByCountry)
  const [shipEnabled, setShipEnabled] = useState(initialShipMap != null)
  const [shipMap, setShipMap] = useState<ShippingRateMap | null>(initialShipMap)
  ...
  async function onSubmit(data: FormData) {
    ...
    const shippingByCountryPayload: ShippingRateMap | null =
      isPhysicalLike && shipEnabled && shipMap && Object.keys(shipMap).length > 0
        ? shipMap
        : null
