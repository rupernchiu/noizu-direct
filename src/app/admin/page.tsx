...
export default async function AdminOverviewPage() {
  ...
  const [userCount, creatorCount, revenueAgg, pendingPayouts, recentOrders, openDisputes] = await Promise.all([
    prisma.user.count(),
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Critical Alerts */}
      {(openDisputes > 0 || pendingPayouts > 0) && (
        <div className="flex flex-wrap gap-3 mb-6">
          {openDisputes > 0 && (
            <Link href="/admin/disputes" className="flex items-center gap-3 rounded-xl bg-destructive/10 border border-destructive/30 px-4 py-3 text-sm hover:bg-destructive/15 transition-colors">
              <Scale className="size-4 text-destructive" />
              <span className="font-semibold text-destructive">{openDisputes}</span>
              <span className="text-muted-foreground">dispute{openDisputes !== 1 ? 's' : ''} awaiting review</span>
              <span className="text-destructive text-xs ml-auto">Review →</span>
            </Link>
          )}
          {pendingPayouts > 0 && (
            <Link href="/admin/payouts" className="flex items-center gap-3 rounded-xl bg-warning/10 border border-warning/30 px-4 py-3 text-sm hover:bg-warning/15 transition-colors">
              <CreditCard className="size-4 text-warning" />
              <span className="font-semibold text-warning">{pendingPayouts}</span>
              <span className="text-muted-foreground">payout{pendingPayouts !== 1 ? 's' : ''} pending</span>
              <span className="text-warning text-xs ml-auto">Process →</span>
            </Link>
          )}
        </div>
      )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Link href="/admin/payouts">
          <div className="bg-card rounded-xl p-4 border border-border hover:border-primary/30 transition-colors cursor-pointer">
